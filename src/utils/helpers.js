...
export function salesFromOrders(orders) {
  return (orders || []).map((order) => {
    ...
    return {
      ...
      paymentStatus: commissionRecord?.payment_status || (order.commission_payment_method ? 'paid' : 'unpaid'),
      paymentMethod: commissionRecord?.payment_method || order.commission_payment_method || '',
      paymentReference: commissionRecord?.reference_number || order.commission_reference_number || '',
      receiptPath: commissionRecord?.receipt_path || order.commission_receipt_path || '',
      paidAt: commissionRecord?.paid_at || order.commission_payment_recorded_at || null,
